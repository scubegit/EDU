...
private static final Logger logger = LoggerFactory.getLogger(EmployerServiceImpl.class);
	...
	@Override
	public List<EmployerVerificationDocResponse> getOneMonthVerificationDocsDataByUserid(long userId, String fromDate, String toDate ) throws Exception {
		
		logger.info("********EmployerServiceImpl getOneMonthVerificationDocsDataByUserid");
		logger.info("------------"+ userId + fromDate + toDate);
		
		List<EmployerVerificationDocResponse> responseList = new ArrayList<>();
		...
		for(VerificationRequest req: list) {
			...
			EmployerVerificationDocResponse resp = new EmployerVerificationDocResponse();
