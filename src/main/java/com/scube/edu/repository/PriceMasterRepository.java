...
public interface PriceMasterRepository extends JpaRepository<PriceMaster, Long>{
	...
	@Query(value = "select * from pricemaster where (select (?1 - year_of_passing) as diff from passing_year_master where id = ?2) between year_range_start and year_range_end and rquest_type_id=?3 and doc_type_id=?4 and is_deleted='N' ", nativeQuery = true)
